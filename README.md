...
- **セキュリティ強化のmanagerフラグ**(いずれも既定は無効=後方互換): `--watch-namespaces`(監視namespaceを限定しClusterRoleでなくnamespace別RoleBindingで縛る)、`--allowed-image-registries`(`spec.image`/`objectStorage.image`/`MisskeyChannel.image`のレジストリprefix許可リスト、webhookで拒否)、`--allowed-cluster-issuers`(`ingress.issuerRef`のClusterIssuer許可リスト)。Ingress annotationはnginx snippet系/auth系を遮断し、operator管理キーはユーザ指定で上書きできません。
